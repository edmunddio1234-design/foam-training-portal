...
const Infographic: React.FC<InfographicProps> = ({ type, title, sections, practice }) => {
  ...
  const [overviewVisual, setOverviewVisual] = useState<string | null>(null);
      ...
      <div className="h-full flex flex-col items-center justify-center p-4 md:p-12 animate-in fade-in zoom-in-95 duration-500">
        ...
        >
          ...
          <div className="absolute inset-0 transition-opacity duration-700">
             {overviewVisual ? (
                <img src={overviewVisual} alt="Infographic Overview" className="w-full h-full object-cover opacity-60 group-hover:opacity-40 transition-opacity" />
             ) : (
    ...
    <div className="space-y-12 overflow-y-auto max-h-[75vh] hide-scrollbar pb-16 px-4 animate-in slide-in-from-bottom-8 duration-700">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="text-center md:text-left space-y-2">
          ...
          <p className="text-slate-500 font-medium max-w-2xl">Interactive Model: Select a component to view standards and AI-generated visual metaphors.</p>
        </div>
        <button 
...
const DetailModal: React.FC<{ section: SubSection; onClose: () => void; color: string }> = ({ section, onClose, color }) => {
  ...
  const [isVisualFullView, setIsVisualFullView] = useState(false);
         ...
         <div className={`w-full md:w-5/12 ${color} p-10 flex flex-col items-center justify-center text-white relative`}>
            {generatingImg ? (
              <div className="flex flex-col items-center gap-4 text-white/60">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">Generating Concept...</span>
              </div>
            ) : aiImage ? (
              <div className="space-y-6 w-full flex flex-col items-center">
                <button 
                  onClick={() => setIsVisualFullView(true)}
                  className="group relative w-full cursor-zoom-in"
                >
                  <img src={aiImage} alt={section.title} className="w-full object-cover rounded-2xl shadow-lg border-4 border-white/20 transition-transform duration-500 group-hover:scale-[1.02]" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-2xl">
                     <i className="fas fa-search-plus text-white text-3xl"></i>
                  </div>
                </button>
                <p className="text-[10px] text-white/60 font-black uppercase tracking-[0.2em] text-center">AI Concept Visual</p>
                <button onClick={generateAIVisual} className="text-[10px] text-white/50 hover:text-white underline mx-auto block">Regenerate Concept</button>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-4">
                <i className={`fas ${section.graphic || 'fa-image'} text-[10rem] opacity-20`}></i>
            ...
            <div className="space-y-6">
               ...
               <div className="grid grid-cols-1 gap-3">
                 {section.details.map((point, i) => (
                   <div 
                     key={i} 
                     onMouseEnter={() => setHoveredDetailIdx(i)}
                   ...
                   >
                      <div className="flex gap-4 items-center">
                        <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black shrink-0 transition-colors ${
                          hoveredDetailIdx === i ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-600'
                        }`}>
                          {i + 1}
                        </div>
